(function(){
    ...
    angular.module("MagazineApp")
        .controller("RegisEntregaDiarioCtrl",  ['$http','uiGridConstants','ngToast','$window',RegisEntregaDiarioCtrl]);

    function RegisEntregaDiarioCtrl($http,uiGridConstants,ngToast,$window,hotkeys){
      var vm = this;
      var codCanilla;
      ...
      vm.enter = function(keyEvent) {
        if (keyEvent.which === 13){
            vm.cargar_pauta(vm.codigo,vm.usuario);
        }
      }
      ...
      vm.cargar_pauta = function(codigo,usuario){
        vm.gridOptions.data = [];
        ...
        //peticion get
        console.log('-------------');
        $http.get("/api/pauta/cargar-entrega/"+codigo+"/0/")
      ...
      vm.enviar_data = function(){
        ...
        if (vm.validado) {
          var dni = 'D0000000'
          $http.post("/api/asignacion/producto-save/"+codCanilla+"/"+dni+"/", vm.gridOptions.data)
