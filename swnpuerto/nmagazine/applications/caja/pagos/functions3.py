...
class Pago():
    ...
    precio_venta = 0.000
    amount = 0.000
    #comentar para la primera migracion
...
def deuda_magazine_canilla(cod_canilla,type_user):
    print '==========='
    print type_user
    ...
    for ld in lista_diarios:
        ...
        if (((tipo=='1') and (verificar_por_cobrar(canilla,detail_guide)))or(tipo=='0')or(type_user=='3')):
            p = Pago()
            #
...
def cerrar_guias():
    ...
    for g in guias:
        #recperamos la deuda
        count_total = DetailAsignation.objects.suma_entregado_guia(g.number)
        if Payment.objects.filter(anulate=False,detail_asignation__asignation__detail_guide__guide=g).exists():
            if Payment.objects.deuda_by_guide(g.number,count_total) <= 0:
                g.culmined = True
                g.save()
                print '===GUIA CERRADA==='
        else:
...
def actualizar_fecha_registro():
    pagos = Payment.objects.all()
    for p in pagos:
        p.date = p.created.date()
        p.save()
