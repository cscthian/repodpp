...
class PaymentManager(models.Manager):
    ...
    def deuda_by_guide(self, guia):
        #valor inicial de suma de movimientos
        mov = 0
        #
        deuda_guia = self.filter(
            anulate=False,
            detail_asignation__asignation__detail_guide__guide=guia,
        ).aggregate(deuda=Sum((F('count_payment')+F('count_return'))))
        if not deuda_guia['deuda'] == None:
            mov = deuda_guia['deuda']

        #calculamos valor entregado
        entregado = DetailAsignation.objects.suma_entregado_guia(guia.number)

        return entregado - mov
