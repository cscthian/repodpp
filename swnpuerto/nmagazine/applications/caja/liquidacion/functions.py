...
#objeto que representa las deudas y pagos de un canilla
class DeudaVendor():
    vendor = ''
    magazine = ''
    date = ''
...
def generar_deudas(fecha):
    #convertimos la fecha a fecha valida
    ...
    resultado = []
    for c in canillas:
        #obtenemos la lista de pagos de la fecha
        pagos = Payment.objects.filter(
            anulate=False,
            created__date=dt,
            invoce__vendor=c,
        ).order_by('detail_asignation__asignation__detail_guide__magazine_day__magazine__tipo')
        #iteramos la los pagos de canillas
        i = 0
        total = 0
        while i < len(pagos):
            p = pagos[i]
            md = p.detail_asignation.asignation.detail_guide.magazine_day
            if p.count_payment > 0 or p.count_return > 0:

                dv = DeudaVendor()
                if i == 0:
                    dv.vendor = c.cod+'-'+c.name + '('+c.seudonimo+')'+'('+c.get_type_vendor_display()+')'
                else:
                    dv.vendor = ' '

        #
                dv.magazine = p.detail_asignation.asignation.detail_guide.magazine_day
                dv.date = p.detail_asignation.created.date()
                dv.entregado = p.detail_asignation.count
                dv.devuelto = p.count_return
                dv.pagado = p.count_payment
                dv.debe = dv.entregado - dv.devuelto - dv.pagado
                dv.precio_unitario = p.detail_asignation.precio_venta
                dv.monto_pagado = p.amount
                dv.monto_deuda = dv.debe*dv.precio_unitario
                total = total + dv.monto_pagado
    ...
    for p in list_proveedores:
        ...
        for pago in pagos_resumen:
            if pago['proveedor'] == p:
                ...
                #asignamos le estado
                if Guide.objects.get(number=pago['guia'], anulate=False).culmined == True:
                    resumen_pagos.estado = 'CERRADO'
                    obj.cerradas = 1
                else:
                    resumen_pagos.estado = 'PENDIENTE'
                    obj.no_cerradas = 1
                #
                # asiganmos datos de objeto resumen_pagos
